...
\item User-level tests or performance measurements of the Grid infrastructure
should be run from a standard user
...
\item User-level tests or performance measurements that interact with Grid services
...
\item User-level tests should be
written and configured using information directly from user documentation
(e.g., hostnames, ports, pathnames, etc.) to emulate a regular user.  
...
\item The configuration of user-level tests or performance measurements should be managed
...
\item User-level tests or performance measurements should be executed locally on all Grid
...
the defined guidelines.  At that time, the only available user-level Grid
