...
grid infrastructure that they are run on.  In order for the data gathered to
be useful, it must gathered repeatedly over time and stored in a manner
appropriate for querying.  In addition to storing the measurements available
we also required that any error messages encountered be stored as well.    
In order to
...
\begin{figure}[tbp]
  ...
  \caption{\label{errorHist_fig} Histogram of Error Messages Encountered.}
\end{figure}

\subsubsection{Results}

GrASP, using Inca, was deployed on TeraGrid and Geon for a period of over 6
...
By the running of the GrASP probes over such an extended period of time
we were able to identify a recurring Globus 2 bug.  Figure ~\ref{errorHist_fig} shows the types (shortened) and number of system errors that were detected while executing the
benchmark during the GrASP deployment on TeraGrid in 2005. The highest occurring
error was number 9, Assertion GLOBUS FALSE, which was the result of a bug in
the Globus 2 GRAM server.  We were also able to predict the
execution time of Preco, a Grid application run on the TeraGrid within 12\% (excluding setup and cleanup time).
