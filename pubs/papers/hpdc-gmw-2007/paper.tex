...
and services is referred to as the Coordinated TeraGrid Software and Services
...
account.  It is important to not execute from a system administrator's account,
since it may have special privileges and use custom shell initialization files.
...
time because of portability issues and incomplete user
...
or a faulty system.  As such, this can be a labor-intensive activity, making it
difficult to achieve full test coverage.  In practice, testing all the commands
listed in the user documentation and/or having at least one basic test for each
software component of the Grid infrastructure typically yields a representative
set of tests.
...
feature of Inca 2; other items note improvements to Inca 1 features.
...
Inca was designed to distribute the control and storage workload among
...
of a suite with another deployment as shown in Figure~\ref{tg_coord_fig}.
Grid B that has its own resources but also has access to resources in Grid A.
When an Inca administrator submits a suite to Grid B, the Inca agent running
on Grid B will determine which reporters run from Grid A resources and forward
...
forwards results back to Grid B's depot.  
...
first set of files is executed in parallel.  Once again all transfers times, as
well as compute and queue times are recorded.

The GrASP probes yield useful data on the performance and reliability of the
Grid infrastructure that they are run on.  As such, data must be gathered 
...
In order to
fulfill the last two requirements, the GrASP probes were modified
...
months.  The data gathered during that period yielded a paper~\cite{grasp2}.
One result was that a recurring Globus 2 bug was identified as shown in Figure ~\ref{errorHist_fig}.  The figure shows the types (shortened) and number of system errors that were detected while executing 
...
the Globus 2 GRAM server.  Furthermore, the execution time of Preco, a Grid application run on the TeraGrid, was predicted within 12\% (excluding setup and cleanup time).

\SubSection{Other Uses}

Other Inca users include the European DEISA Grid and the UK National Grid
Service (NGS).  Similar to TeraGrid, DEISA uses Inca to verify their Common
...
NGS currently has Inca 2 deployed on 8 resources to verify their Grid services 
and compilers.  They have wrapped the GITS~\cite{gits} tests using a new
API developed by the GITS developer.  The GITS reporters include 17 tests 
that verify Globus and GSI-SSH functionality such as executing small job
submissions and data transfers.  NGS also has 12 tests for their compilers and
