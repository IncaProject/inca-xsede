...
% some unnecessary whitespace still -- these suggestions from
...
\item Emulates a regular user by using tests and performance measurements created and configured based on user
documentation rather than on system administrator knowledge (of hostnames,
ports, pathnames, etc.).  This may not always be possible when documentation
and tests are developed simultaneously during pre-production, but they should
be closely coordinated activities.  
...
updated when Grid infrastructure is upgraded.  Also, it is difficult to write
and deploy a test correctly the first time due to portability issues and
incomplete user documentation.  Multiple iterations are usually required to
determine whether a detected failure stems from a faulty test, user
documentation, or Grid resource.  
...
reduce the impact on resources if needed. *
...
SSL, and sensitive information is encrypted before being stored to disk.  To
...
%\begin{figure}[tbp]
%  \centering
%  \includegraphics[width=.4\textwidth]{status-page.eps}
%  \caption{\label{status_page_fig} Portion of the TeraGrid CTSSv3 detailed status page with the resources anonymized.}
%\end{figure}

\begin{figure*}[tbp]
\subfigure[]{\includegraphics[width=.8\columnwidth]{tg-deploy.eps}}
\subfigure[]{\includegraphics[width=.6\textwidth]{status-page.eps}}
...
2003 using Inca 1; TeraGrid transition to Inca 2 completed in
...
users on the types of statistics and graphs that will prove to be most useful.
