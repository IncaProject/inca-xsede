...
`wget --no-check-certificate https://software.xsede.org/security/xsede-certs.tar.gz 2>&1`;
`tar zxvf xsede-certs.tar.gz`;
my $out = `diff -x '*.r0' --exclude-from=/etc/grid-security/exclude.txt -r certificates /etc/grid-security/certificates`;
if ( $? != 0 ) {
  ...
  `cp /etc/grid-security/InCommon-Server-CA/* /etc/grid-security/certificates`;
   open( MAIL, '| mail -s "quarry certs were refreshed" ssmallen@sdsc.edu') || die "Unable to mail";
   print MAIL $out;
   close MAIL;
} 
`rm -fr $dir`;
