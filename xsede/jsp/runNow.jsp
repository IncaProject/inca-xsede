...
<%-- Check permissions --%>
<c:import var="adminConfigString" url="/xml/admin.xml"/>
...
<c:choose><c:when test="${empty xml || ! fn:contains(xml, 'series')}">
  ...
  <x:parse var="sc" xml="${xml}"/> 
  <c:set var="nickname"><x:out select="$sc//series/../nickname"/></c:set>
  <c:set var="reporter"><x:out select="$sc//name"/></c:set>
