...
<xsl:stylesheet version="2.0"
  ...
  <xsl:template name="printSeriesResultsTable">
    ...
    <xsl:variable name="groupregex">
      <xsl:choose><xsl:when test="$defaultconfig/group"><xsl:value-of select="string-join($defaultconfig/group/@regex, '|')"/></xsl:when><xsl:otherwise>^$</xsl:otherwise></xsl:choose>
    </xsl:variable>
    ...
    <table class="subheader">
      <xsl:for-each select="$defaultconfig/group">
        <xsl:variable name="regex" select="@regex"/>
        <xsl:variable name="strip" select="@strip"/>
        <xsl:if test="count($groupedseries[matches(.,$regex)])>0">
        <tr>
        ...
        <xsl:for-each select="$groupedseries[matches(.,$regex)]">
          ...
          <xsl:call-template name="printSeriesResultsRow">
            ...
            <xsl:with-param name="printSeriesName" select="replace($seriesName,$strip,'')"/>
    ...
    <xsl:choose><xsl:when test="count($result)=1">
      ...
      <xsl:variable name="normRef">
        <xsl:choose><xsl:when test="$result/gmt">
          <xsl:value-of select="concat('/inca/jsp/instance.jsp?nickname=', encode-for-uri($result/nickname), '&amp;resource=', $result/hostname, '&amp;collected=', $result/gmt)"/>
        </xsl:when><xsl:otherwise>
