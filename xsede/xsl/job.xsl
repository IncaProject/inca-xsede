...
<xsl:stylesheet version="1.0"
  ...
  <xsl:template name="suite">
    ...
    <xsl:variable
        name="resources"
        select="suiteResults/resourceConfig/resources/resource[name]"/>
  ...
  <xsl:template name="getResults">
    ...
    <xsl:variable name="result" select="//suite//reportSummary[matches(hostname,
    ...
    <td class="clear">
      ...
      <xsl:variable name="href">
        <xsl:value-of select="'xslt.jsp?xsl=job-history.xsl'"/>
        <xsl:value-of select="'&amp;configID='"/>
        <xsl:value-of select="$conf"/>
      </xsl:variable>
      <p class="footer"><xsl:text>[ </xsl:text>
        <a href="{$href}"><xsl:text>history</xsl:text></a>
        <xsl:text> ]</xsl:text></p>
    </td>
    <xsl:choose>
      <xsl:when test="$uri!=''">
        ...
        <xsl:variable name="href"
            ...
            $instance,'&amp;configID=',$conf,'&amp;resourceName=', $machine)"/>
        <xsl:choose>
          <xsl:when test="$exit!=''">
            <td class="{$exit}">
