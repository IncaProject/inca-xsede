...
<xsl:stylesheet version="2.0"
    ...
    <xsl:template name="generateHTML" match="combo">
        ...
        <td align="right"><xsl:call-template name="tg-menu" /></td></tr></table>
        <!-- printSuiteInfo -->
    ...
    <!-- resultsAllPackages                                                   -->
    <!--                                                                      -->
    <!-- Calls template to print category packages                            -->
    <!-- ==================================================================== -->
    ...
    <xsl:template name="printPackage" match="package">
        ...
        <tr>
            <td class="subheader"><a name="{$package}"><xsl:value-of select="$package"/></a></td>
    ...
    <xsl:template name="printResultsRow" match="unitalias|version">
        ...
        <xsl:if test="($prodReportersOnly='true' and count(status[.='dev'])=0) or $prodReportersOnly='false'">
            <tr>
                <td class="clear">
    ...
    <xsl:template name="printResourceResultCell" match="resource" mode="result">
        ...
        <xsl:variable name="regexHost" select="concat($thisResource, '|', replace(macros/macro[name='__regexp__']/value, ' ','|'))"/>
        <xsl:variable name="result" select="$suite/reportSummary[matches(hostname, $regexHost) and nickname=$testname]" />
        ...
        <xsl:choose>
            <xsl:when test="count($result)>0">
                ...
                <xsl:variable name="href" select="concat('xslt.jsp?xsl=instance.xsl&amp;instanceID=', $instance, '&amp;configID=', $result/seriesConfigId)"/>
                ...
                <xsl:variable name="exit">
                    <xsl:choose>
                        <xsl:when test="$tickets/ticket[matches(resource, $thisResource)]">
                            ...
                            <xsl:value-of select="$tickets/ticket[matches(resource, $thisResource)]/number" />
                        </xsl:when>
                        <xsl:when test="$package/packagewait[matches(resource, $thisResource)]">
                            <xsl:value-of select="'pkgWait'" />
                        </xsl:when>
                        <xsl:when test="$package/incawait[matches(resource, $thisResource)]">
                            <xsl:value-of select="'incaWait'" />
                        </xsl:when>
                        <xsl:when test="string($instance)=''">
                            <xsl:value-of select="''" />
                        </xsl:when>
                        <xsl:when test="string($result/body)!='' and string($result/errorMessage)='' and ($comparitor='Success' or count($comparitor)=0)">
