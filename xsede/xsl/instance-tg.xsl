...
<xsl:stylesheet version="1.0"
    ...
    <xsl:template name="printReport" match="report">
        ...
        <xsl:variable name="cgi" select="concat(substring-before($config/series/uri, name), '../cgi-bin/reporters.cgi?reporter=', name, '&amp;action=help')"/>
        ...
        <xsl:variable name="age" select="replace(replace(replace(replace(replace(current-dateTime() - $gmt, 'P', ''), 'D', ' days '), 'T', ''), 'H', ' hours '), 'M.*', ' minutes')" />
        ...
        <table width="600" cellpadding="4">
            <tr><td colspan="2"><h3><xsl:value-of select="concat('Details for ', $nickName, ' series')"/></h3></td></tr>
            <tr><td colspan="2" class="header"><xsl:text>Result:</xsl:text></td></tr>
            <tr>
                <td colspan="2">
                    <p><xsl:value-of select="$resultText"/></p>
                    <xsl:if test="$resultText=$comp">
                        <p class="code"><xsl:value-of select="concat('Expecting: ', $config/acceptedOutput/comparison)"/></p>
                    </xsl:if>
                    <xsl:if test="string($package/version)!=''">
                        <p class="code"><xsl:value-of select="concat('Found: ', $package/version)"/></p>
                    </xsl:if>
            ...
            <tr><td colspan="2" class="header"><xsl:text>Reporter details:</xsl:text></td></tr>
            <tr>
            ...
            <tr>
                ...
                <td><xsl:value-of select="sdf:format(sdf:new('MM-dd-yyyy hh:mm a (z)'), $gmt)" /></td>
            </tr>
            <tr>
            ...
            <tr><td colspan="2" class="header"><xsl:text>Input parameters:</xsl:text></td></tr>
            <xsl:for-each select="$config/series/args/arg">
                <xsl:sort/>
                <tr><td><xsl:value-of select="name"/></td>
                    <td><xsl:value-of select="value"/></td></tr>
            </xsl:for-each>
            <tr><td colspan="2" class="header"><xsl:text>Command used to execute the reporter:</xsl:text></td></tr>
            <tr><td colspan="2"><p class="code">
                <xsl:value-of select="concat('% ', replace($config/series/context, $config/series/name, reporterPath))"/>
            </p></td></tr>
            <xsl:if test="count(log/system/message)>0">
                <tr><td colspan="2" class="header"><xsl:text>System commands executed by the reporter:</xsl:text></td></tr>
                <tr>
            ...
            <xsl:if test="$url[not(matches(., 'noComment'))]">
                <tr><td colspan="2" class="header"><xsl:text>Comments:</xsl:text></td></tr>
                <tr><td colspan="2">
                    <xsl:choose>
                    ...
                    <form method="post" action="{replace(replace(replace($page, ':8080/', ':8443/'), 'http://', 'https://'), 'xslt.jsp', 'comments.jsp')}">
