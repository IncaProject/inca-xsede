...
<xsl:stylesheet version="2.0"
        ...
        <xsl:variable name="title">
            <xsl:choose>
                <xsl:when test="count(suiteResults)=1">
                    <xsl:value-of select="concat('Inca Results for ', suiteResults/suite/name)"/>
                </xsl:when>
                <xsl:otherwise>
                    <xsl:value-of select="''"/>
                </xsl:otherwise>
    ...
    <xsl:template name="printSuiteInfo" match="suite">
        ...
        <xsl:variable name="seriesNames" select="distinct-values(reportSummary/nickname)"/>
        <!-- inca-common.xsl -->
    ...
    <xsl:template name="printResourceResultCell" match="resource" mode="result">
        ...
        <xsl:variable name="regexHost" select="concat(name, '|', replace(macros/macro[name='__regexp__']/value, ' ','|'))"/>
            ...
            <xsl:when test="count($result)>0">
                ...
                <xsl:choose>
                    <xsl:when test="$exit!=''">
                        <td class="{$exit}">
                            <a href="{$href}">
                                <xsl:variable name="stats" select="$result/body/performance/benchmark/statistics" />
                                <xsl:variable name="depth" select="$stats/statistic[matches(., 'depth')]/value" />
                                <xsl:variable name="mdshost" select="$stats/statistic[matches(., 'hostname')]/value" />
                                <xsl:choose>
