...
<xsl:stylesheet version="2.0"
  ...
  <xsl:template name="printSummaryTable">
    ...
    <table cellpadding="8" class="subheader">
      <xsl:for-each select="$resources">
        ...
        <xsl:variable
            name="unitMatch"
            select="//rs:reportSummary[matches(hostname,
          ...
          <td class="clear" valign="top">
            ...
            <xsl:if test="$numUnitVersionFail>0">
              ...
              <ol><xsl:for-each select="$unitVersionFail">
                ...
                <xsl:variable
                    name="result"
                    select="//rs:reportSummary[matches(hostname, $regexHost)
                    and nickname=$thisTest]" />
                <xsl:call-template name="printTestLink" >
                  <xsl:with-param name="resourceName" select="$resourceName"/>
                  <xsl:with-param name="reportSummary" select="$result"/>
                </xsl:call-template>
              </xsl:for-each></ol>
            </xsl:if>
            <xsl:if test="$numCrossFail>0">
              <xsl:value-of select="concat(' (',$numCrossFail,' cross-site)')"/>
              <ol><xsl:for-each select="$crossFail">
                <xsl:sort select="."/>
                <xsl:call-template name="printTestLink" >
                  <xsl:with-param name="resourceName" select="$resourceName"/>
                  <xsl:with-param name="reportSummary" select="."/>
                </xsl:call-template>
              </xsl:for-each></ol>
            </xsl:if>
          </td>
  ...
  <xsl:template name="printTestLink" match="failure|reportSummary">
    ...
    <xsl:variable name="href" select="concat('/inca/jsp/instance.jsp?nickname=', 
          $reportSummary/nickname, '&amp;resource=', $reportSummary/hostname, '&amp;collected=', $reportSummary/gmt)"/>
    <li><a href="{$href}">
